                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($Teachers as $Teacher)
                                            <tr>
                                                <?php $i++; ?>
                                                <td>{{ $i }}</td>
                                                <td>{{ $Teacher->name ?? 'لا يوجد اسم' }}</td>


                                                <td>{{ $Teacher?->Joining_Date }}</td>

                                                <td>{{ $Teacher?->specializations->Name ?? 'لا يوجد اسم' }}</td>
                                                <td>
                                                    <a href="{{ route('admin.Teachers.show', $Teacher->id) }}"
                                                        class="btn btn-primary btn-sm" role="button"
                                                        aria-pressed="true"><i class="fa fa-eye"></i>
                                                    </a>
                                            ...
                                            <div class="modal fade" id="delete_Teacher{{ $Teacher->id }}"
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <form action="{{ route('admin.Teachers.destroy', 'test') }}"
                                                        method="post">
                                                        @method('DELETE')
                                                        @csrf
