...
<div class="row">
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                <a href="{{ route('teacher.package.create') }}" class="btn btn-success" role="button"
                        ...
                        <tbody>
                            @foreach ($packages as $package)
                                <tr>
                                    ...
                                    <td>{{ $package->grade ? $package->grade->Name : 'N/A' }}</td>
                                    <td>{{ $package->class_room ? $package->class_room->Name_Class : 'N/A' }}</td>

                                    <td>
                                        @if($package->package_type ==1)
                                        <span class="badge badge-info"> اسئله فقط</span>

                                        @elseif($package->package_type ==2)
                                        <span class="badge badge-info"> شرح فقط</span>

                                        @else 
                                        <span class="badge badge-info"> اسئله وشرح </span>
                                    ...
                                    <td>
                                        <a
                                        href="{{ route('teacher.package.edit', $package->id) }}" class="btn btn-sm btn-primary">
                                        <i
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{route('teacher.online_sessions.destroy',$package->id)}}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <div class="modal fade" id="Delete_package{{$package->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{route('teacher.package.destroy',$package->id)}}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        <input type="hidden" name="meeting_id" value="{{$package->meeting_id}}">
                                                        <h5 style="font-family: 'Cairo', sans-serif;">هل انت متاكد مع عملية الحذف ؟</h5>
